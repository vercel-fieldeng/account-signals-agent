...
describe("owner-bound autonomous diagnostic", () => {
  ...
  it("uses one d0 request followed by bounded optional Salesforce and Index enrichment", () => {
    ...
    expect(prompt).toContain("reinforces an existing motion")
    ...
    for (const field of ["*Account:*", "*Signal:*", "*Context:*", "*Hypothesis:*", "*Contacts:*", "*Next step:*"]) {
      expect(prompt).toContain(field)
    }
    expect(prompt).toContain("Do not include a Date field")
