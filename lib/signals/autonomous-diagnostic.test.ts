...
describe("owner-bound autonomous diagnostic", () => {
  ...
  it("calculates the preceding seven complete UTC days and defines the full verified account roster", () => {
    ...
    expect(prompt).toContain("FIRST resolve and verify each exact Account ID")
    ...
    expect(prompt).toContain("this is the full verified union, not a sample")
    ...
    expect(prompt).toContain("longest source-supported daily usage history")
    expect(prompt).toContain("collect_external_signals")
    expect(prompt).toContain("baselineAvailable: false")
    expect(prompt).toContain("external-source status and limitations")
    expect(prompt).toContain("project_usage_metrics_daily_rollup_t180d")
    expect(prompt).toContain("up to 180 trailing days")
    expect(prompt).toContain("teams_daily.usage_trend_l30d")
    expect(prompt).toContain("Request weekly or other compact aggregates")
