...
describe("owner-bound autonomous diagnostic", () => {
  ...
  it("uses one bounded d0-only request and a useful result contract", () => {
    ...
    expect(prompt).toContain("Do not call collect_external_signals or Salesforce")
    expect(prompt).toContain("CRM ownership not checked")
    expect(prompt).toContain("No surfaced signals returned for this window")
    ...
    expect(prompt).toContain("CHECK NOW")
    expect(prompt).toContain("*Why now:*")
    expect(prompt).toContain("*Why it matters:*")
    expect(prompt).toContain("*Next:*")
    expect(prompt).toContain("DETAIL:")
    expect(prompt).toContain("Signal evidence")
    expect(prompt).toContain("grouped under its account")
