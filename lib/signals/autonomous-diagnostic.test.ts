...
describe("owner-bound autonomous diagnostic", () => {
  ...
  it("uses the full Sam Maass SE book and the bounded three-day intent-signal request", () => {
    ...
    expect(prompt).toContain("Do not perform a Salesforce roster lookup before d0")
    expect(prompt).toContain("Scope the d0 request to Sam Maass's SE book")
    expect(prompt).toContain("this is the full requested SE book, not a sample")
    ...
    expect(prompt).toContain("use a hardcoded name/ID list")
    ...
    expect(prompt).toContain("Start the d0 invocation immediately")
