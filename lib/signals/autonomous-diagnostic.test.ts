...
describe("owner-bound autonomous diagnostic", () => {
  ...
  it("calculates the preceding seven complete UTC days and includes the ownership gate and five accounts", () => {
    ...
    expect(prompt).toContain("FIRST verify each exact Account ID")
    expect(prompt).toContain("Sam Maass as SA OR Stefan Nikolic as AE")
    expect(prompt).toContain("Engel & Völkers")
    expect(prompt).toContain("SumUp Germany")
    expect(prompt).toContain("Air Up GmbH")
    expect(prompt).toContain("Nintendo of Europe GmbH")
    expect(prompt).toContain("onvista media GmbH")
    ...
    expect(prompt).toContain("at most 1,800 characters")
