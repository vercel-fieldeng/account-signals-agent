...
Scope all distinct Salesforce Accounts from the authoritative current union of Sam Maass's SA accounts OR Stefan Nikolic's AE accounts; this is the full verified union, not a sample. Resolve the roster at runtime from Salesforce Account records. Do not use a hardcoded name/ID list, d0-only selection, downstream signals, or websites as roster keys. If no accounts can be verified, report the exact count and why; never invent or substitute accounts.
