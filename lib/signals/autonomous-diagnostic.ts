...
Scope all distinct Salesforce Accounts from Sam Maass's authoritative current SE book; this is the full verified SE book, not a sample. Resolve the full roster at runtime with one bounded Salesforce Account lookup that returns each exact Account ID, current name, and authoritative assignment to Sam Maass as Solutions Engineer. Do not make one Salesforce lookup per account unless a surfaced candidate needs context. Do not use a hardcoded name/ID list, d0-only selection, downstream signals, or websites as roster keys. If no accounts can be verified, report the exact count and why; never invent or substitute accounts.

Use the single roster result to verify the exact Salesforce user identity, Account IDs, current names, and assignment field. Generic account-team membership is not proof of the assignment. Deduplicate the verified Sam Maass SE book by Salesforce Account ID. If verification is unavailable or outside that book, exclude the account and report it; never retrieve signals for excluded accounts. Owner scope is a required instruction, not proof of deterministic authorization enforcement.
