...
export function isPendingDiagnosticMessage(message: string | null | undefined): boolean {
  ...
  return message.includes("Status: WAITING_FOR_D0") || message.includes("Status: WAITING_FOR_CONTEXT") || message.includes("Signal retrieval is still processing") || message.includes("No signal brief yet")
}
