...
- Put source coverage, external-source status and limitations, contextual checks, suppressed expected candidates, hypotheses, freshness, hiring, LinkedIn, and lifecycle caveats in the `DETAIL:` reply. Unknown or unavailable coverage is not none. If any required source is still working, incomplete, or failed, mark the result `PARTIAL` or `BLOCKED` and name the limitation; never call it `No actionable findings` until all required sources completed successfully. Keep the reply concise and evidence-led; do not include SQL, raw rows, internal IDs, or credentials.
