...
- Start `BLUF:` with a dynamic outcome headline, then one compact line containing status, `1d`, signal count, account count, and context coverage. Keep the full BLUF under 2,000 characters and each account card under 420 characters.
...
- `scan_account_news` is the only public-news path in the daily brief. It returns only judge-rated material Opportunity or Risk events for the account watchlist, excluding delivered ones; report them as returned, never add your own web findings, and omit news entirely when it returns none. List every shown event id on the Reported signal IDs line.
