...
The diagnostic now uses a bounded 15-account live sample, not whole-book coverage. It resolves the authoritative Salesforce union of Sam Maass SA accounts and Stefan Nikolic AE accounts at runtime, deduplicated by Account ID and selected deterministically by LastActivityDate, Name, then Account ID when the union exceeds 15. It requires verifying each selected exact Salesforce ID, current name, and assignment before signal retrieval. Generic account-team membership is not proof of either assignment. Consumption comparisons use seven UTC calendar days and the preceding seven days, but d0 should also provide a compact longer baseline from the source-supported `project_usage_metrics_daily_rollup_t180d` history, up to 180 trailing days, plus available `teams_daily` trend/breadth signals. The 7-day comparison remains primary; long-history output must report its actual dates, grain, freshness, completeness, source cap, and truncation rather than becoming an unbounded raw extract. Calendar coverage is reported separately from source finality; a settling final day keeps current values as lower bounds and blocks stable/declining labels. The pilot currently validates Salesforce plus d0 consumption/adoption only; hiring and LinkedIn remain unknown/partial-scope without an approved non-authenticated source.
