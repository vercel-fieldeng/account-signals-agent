...
The diagnostic asks d0 directly for up to ten recent surfaced intent-signal rows using the existing semantic alert `New intent signals — Sam Maass SE book`, scoped as `SE = Sam Maass`, and an exact rolling three-day UTC window. It explicitly does not require a physical source column named `sales_engineer_name`. The scheduler preflight validates only the persisted operator's d0 grant. Salesforce roster lookup, `collect_external_signals`, careers, news, and web enrichment are intentionally excluded from this critical path. A completed d0 result must always produce a triage brief: a dynamic outcome headline, one compact status/count line, and at most three `CHECK NOW`, `INVESTIGATE`, or `WATCH` account cards with `Why now`, `Why it matters`, and one imperative `Next` check. DETAIL groups every returned row by account with human-readable dates and one shared coverage footer containing the exact UTC window, freshness/completeness, truncation, and aggregate flip/fetch status. Missing CRM ownership is shown once as not checked rather than suppressing d0 evidence.
